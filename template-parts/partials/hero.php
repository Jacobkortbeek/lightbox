<div class="hero" style="background-image: url('<?php the_sub_field( 'background_image' ); ?>">
  <div class="container-fluid fullHeight fullCenterColumnEven" >
    <div class="row">
      <div class="hero-title col-md-8 offset-md-3 mx-auto">
        <h1><?php the_sub_field('hero_title'); ?></h1>
      </div>
    </div>
    <div class="row">
      <div class="hero-subtitle col-md-8 offset-md-3 mx-auto prox">
        <h3><?php the_sub_field('hero_subtitle'); ?></h3>
      </div>
    </div>
    <div class="row">
      <div class="hero-play-button col-md-8 offset-md-3 mx-auto fullCenter">
        <img src="<?php the_sub_field('play_button'); ?>" alt="play button">

      </div>
    </div>
    <div class="row">
      <div class="hero-play-text col-md-8 offset-md-3 mx-auto prox">
        <h3><?php the_sub_field('play_video_text'); ?></h3>
      </div>
    </div>
    <div class="row">
      <div class="hero-title col-md-8 offset-md-3 mx-auto fullCenter">
        <img src="<?php the_sub_field('down_arrow_image'); ?>" alt="down arrow">
      </div>
    </div>
  </div>
</div>
